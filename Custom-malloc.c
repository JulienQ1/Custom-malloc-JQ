        ...
        for (size_t i = 0; i < freed_chunks.count; ++i) {
            ...
            if (chunk.size >= size_words) {
                ...
                if (tail_size_words > 0) {
                 ...
                 // If no suitable chunk was found, increase the program's data segment size
        void *new_chunk = sbrk(size_words * sizeof(uintptr_t));
        if (new_chunk == (void *) -1) {
            return NULL; // sbrk failed
        } else {
            chunk_list_insert(&alloced_chunks, new_chunk, size_words);
            return new_chunk;
        }
    }
